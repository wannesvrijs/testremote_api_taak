...
if ( $_POST["savebutton"] == "Save" )
{
    ...
    if ( $_POST[$pkey] > 0 ) //update
    {
        ...
        if ( $DBM->ExecuteSQL($sql) ) $new_url = $_application_folder  . "/$formname.php?id=" . $_POST[$pkey] . "&updateOK=true" ;
    }
    else //insert
    {
        $sql = "INSERT INTO $tablename SET " . implode( ", " , $sql_body );
        if ( $DBM->ExecuteSQL($sql) ) $new_url = $_application_folder . "/$afterinsert?insertOK=true" ;
    }
